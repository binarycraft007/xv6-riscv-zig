...
pub fn create(owner: *std.Build, syscalls: []const []const u8) *SyscallGenStep {
    const self = owner.allocator.create(SyscallGenStep) catch @panic("OOM");
...
pub fn getFileSource(self: *SyscallGenStep) std.Build.FileSource {
...
fn make(step: *Step, prog_node: *std.Progress.Node) !void {
    ...
    for (self.syscalls) |syscall| {
        try output.appendSlice(
            try mem.concat(gpa, u8, &[_][]const u8{
                ".global ",
                syscall,
                "\n",
                syscall,
